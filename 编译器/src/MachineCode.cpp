...
#include "Type.h"
extern FILE* yyout;
int MachineBlock::label = 0; //初始化label,用来判断是否达到500个指令
    ...
    * lable addr_a -> print addr_a; */
    ...
    case LABEL:
        if (this->label.substr(0, 2) == ".L" || is_func){
            if(is_func){ //是函数的时候输出要去掉‘@’
                fprintf(yyout, "%s", this->label.erase(0,1).c_str());
            }else{
                fprintf(yyout, "%s", this->label.c_str());
            }
        }
        else //变量的输出要去掉‘@’
            fprintf(yyout, "addr_%s%d", this->label.erase(0,1).c_str(), parent->getParent()->getParent()->getParent()->getN());
    default:
        break;
    }
}
...
{
    fprintf(yyout, "\tldr ");
    this->def_list[0]->output(); //load指令的目的地址输出
    fprintf(yyout, ", ");
    ...
    this->use_list[0]->output(); //load的原操数作地址输出
...
StackMInstrcuton::StackMInstrcuton(MachineBlock* p, int op, std::vector<MachineOperand*> srcs, MachineOperand* src,
                MachineOperand* src1, int cond) {
    this->parent = p;
...
{ 
    ...
    this->stack_size = 0;
    this->paramsNum = ((FunctionType*)(sym_ptr->getType()))->getParamsType().size();
};
//新写的
void MachineBlock::output() {
    bool isfirst = true; //是第一个参数
    int offset = (parent->getSavedRegs().size() + 2) * 4; //寄存器偏移
...
void MachineFunction::output()
{
    ...
    *  4. Allocate stack space for local variable */
    ...
    for (auto iter : block_list) {
        iter->output();
        inst_num += iter->getSize();
        if(inst_num > 160){ //指令个数大于160时，清零重新计数
            fprintf(yyout, "\tb .F%d\n", parent->getN());
            fprintf(yyout, ".LTORG\n");
            parent->printGlobal();
            fprintf(yyout, ".F%d:\n", parent->getN()-1);
...
void MachineUnit::PrintGlobalDecl() {
    ...
    for (long unsigned int i = 0; i < global_list.size(); i++) {
        IdentifierSymbolEntry* se = (IdentifierSymbolEntry*)global_list[i];
            fprintf(yyout, "\t.global %s\n", se->toStr().erase(0,1).c_str());
            fprintf(yyout, "\t.align 4\n");
            fprintf(yyout, "\t.size %s, %d\n", se->toStr().erase(0,1).c_str(), se->getType()->getSize() / 8);
            fprintf(yyout, "%s:\n", se->toStr().erase(0,1).c_str());
            if(se->getType()->isInt()) {
